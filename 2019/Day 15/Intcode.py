class computer:
    ...
    class parametersClass:
        def __init__(self,oneIntcode):
            ...
            self.A = self.checkZeroOrOneOrTwo(arrayParam[0], 'Parameter A')
            self.B = self.checkZeroOrOneOrTwo(arrayParam[1], 'Parameter B')
            self.C = self.checkZeroOrOneOrTwo(arrayParam[2], 'Parameter C')
            self.D = arrayParam[3]
            self.E = arrayParam[4]
        ...
        def checkZeroOrOneOrTwo(self,value,valueNameForErrorMessage):
            if value == 0 or value == 1 or value == 2:
                return value
            else:
                raise ValueError(f'{valueNameForErrorMessage} should be 0, 1 or 2. Not {value}')
    ...
    def expandMemory(self,targetSize):
        while len(self.intcode) < targetSize:
            self.intcode.append(0)

    def checkIfMemoryNeedsExpanding(self,value):
        valuePlusOne = value +1
        if valuePlusOne > len(self.intcode):
            self.expandMemory(valuePlusOne)

    def takeStep(self, position = None):
        def upperBoundExpandMemory(self, targetSize):
            self.expandMemory(targetSize)
            return f'position too high, expanding memory to {targetSize}'
        ...
        if position > len(self.intcode)-1:
            return upperBoundExpandMemory(self,position)
        ...
        if opcode is 1:
            ...
            return self.opcode9(position,currentModes)
        elif opcode is 99:
    ...
    def getValueFromIntcodes(self,position,param):
        if param == 0:
            ...
            self.checkIfMemoryNeedsExpanding(actualPosition)
            result = self.intcode[actualPosition]
        elif param == 1:
            result = self.intcode[position]
        elif param == 2:
            relativePosition = self.relativeBase + self.intcode[position]
            self.checkIfMemoryNeedsExpanding(relativePosition)
            result = self.intcode[relativePosition]
        else:
            raise ValueError('parameter should be 0, 1 or 2 and this should be caught earlier')
        return result
    ...
    def writeResultToIntcodes(self,position,param,result):
        if param == 0:
            ...
            relativePosition = self.relativeBase + self.intcode[position]
            self.checkIfMemoryNeedsExpanding(relativePosition)
            self.intcode[relativePosition] = result
        else:
            raise ValueError('parameter should be 0, 1 or 2 and this should be caught earlier')
    ...
    def opcode9(self,position,modes):
        #adjusts relative base
        first = self.retrieveFirst(position,modes)
        self.relativeBase += first
        self.position += 2
        return f'opcode 9 at position {position} processed'

def constructComputerFromFile(path,automaticMode=False):
    rawData = []
    ...
    result = computer(intcode,automaticMode=automaticMode)
    return result
